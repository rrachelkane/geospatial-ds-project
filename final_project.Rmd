...
start_points_sf <- st_join(start_points_sf, nyc_tract, left = TRUE) %>%
  dplyr::select(start_station_name, start_tract_id = CT2020, start_borough_name = BoroName, geometry)

# same for end
end_points_sf <- st_join(end_points_sf, nyc_tract, left = TRUE) %>%
  dplyr::select(end_station_name, end_tract_id = CT2020, end_borough_name = BoroName, geometry)
...
# check for any NAs
print(sum(is.na(start_points_sf$start_tract_id)))
print(sum(is.na(end_points_sf$end_tract_id)))

# inspect
start_points_sf %>% 
  filter(is.na(start_tract_id)) %>% 
  head() # liberty light rail in NJ? + LA demo?
# only n=2 obs in cb all, so can drop

end_points_sf %>% 
  filter(is.na(end_tract_id)) %>% 
  head()

na_end_tract <- end_points_sf %>% 
  filter(is.na(end_tract_id)) # approx 160 or so obs in cball
...
# convert to dt for efficiency
setDT(start_points_sf)
setDT(end_points_sf)

# calculate interaction with CPZ for start stations: which are within the zone? - double check
start_points_sf <- start_points_sf[, geometry := st_transform(geometry, st_crs(sf.boundary_proj))
][, start_in_cpz := st_within(geometry, sf.boundary_proj, sparse = FALSE)
]

# calculate interaction with CPZ for end stations: which are within the zone? - double check
end_points_sf <- end_points_sf[, geometry := st_transform(geometry, st_crs(sf.boundary_proj))
][, end_in_cpz := st_within(geometry, sf.boundary_proj, sparse = FALSE)
]

# now, given the spatial information of interest has been retrieved, merge back into the origin-destination-peakhour-day trip data
cb_all <- cb_all[start_points_sf, on = "start_station_name"]
cb_all <- cb_all[end_points_sf, on = "end_station_name"]
...
# note might be more efficient way of doing these statistic

# trip counts per tract, all jan 
  ggplot(
    nyc_tract %>%
      left_join(
        cb_all %>%
          group_by(start_tract_id) %>%
          summarise(trip_count = sum(trip_count), .groups = "drop"),
        by = c("CT2020" = "start_tract_id")
      )
  ) +
    geom_sf(aes(fill = trip_count), color = "white", size = 0.1) +  # Fill by trip count
    scale_fill_viridis_c(option = "magma", na.value = "gray90", name = "Trip Count") +
    labs(
      title = "Trip Counts by Census Tract - January 2025",
      subtitle = "Aggregated based on trip start locations",
      caption = "Source: Citibike Data"
    ) +
    theme_minimal()

  # something off here - pretty sure there are not bikes in staten island
  #maybe something to do with crs of trip data
  # and central park?

  
  # percentile plots on trip type shares, peak, etc, heat/corr maps
```